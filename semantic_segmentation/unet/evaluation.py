...
from assets import labels  # noqa: E402
from metrics import Evaluation, confusion_matrix  # noqa: E402
from dataloader import GenDEBRIS, bands_mean, bands_std  # noqa: E402
...
logging.basicConfig(filename=os.path.join(root_path, 'logs', 'evaluating_unet.log'),
...
def main(options):
    ...
    with torch.no_grad():
        ...
        if options['predict_masks']:
            ...
            ROIs = np.genfromtxt(os.path.join(
                root_path, 'data', 'splits', 'test_X.txt'), dtype='str')
            ...
            for roi in tqdm(ROIs):
                ...
                roi_file = os.path.join(
                    path, roi_folder, roi_name + '.tif')     # Get File path

                os.makedirs(options['gen_masks_path'], exist_ok=True)

                output_image = os.path.join(options['gen_masks_path'], os.path.basename(
                    roi_file).split('.tif')[0] + '_unet.tif')
...
if __name__ == "__main__":
    ...
    # Unet model path
    parser.add_argument('--model_path', default=os.path.join(up(os.path.abspath(__file__)),
                        'trained_models', '44', 'model.pth'), help='Path to Unet pytorch model')

    # Produce Predicted Masks
    parser.add_argument('--predict_masks', default=False, action="store_true",
                        type=bool, help='Generate test set prediction masks?')
    parser.add_argument('--gen_masks_path', default=os.path.join(root_path, 'data',
                        'predicted_unet'), help='Path to where to produce store predictions')
