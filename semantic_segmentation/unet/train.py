...
from dataloader import GenDEBRIS, bands_mean, bands_std, RandomRotationTransform, class_distr, gen_weights  # noqa: E402

root_path = up(up(up(os.path.abspath(__file__))))

logging.basicConfig(filename=os.path.join(root_path, 'logs', 'log_unet.log'),
...
def main(options):
    ...
    writer = SummaryWriter(os.path.join(
        root_path, 'logs', options['tensorboard']))
    ...
    if options['mode'] == 'train':
        ...
        dataset_train = GenDEBRIS('train', transform=transform_train,
                                  standardization=standardization, agg_to_water=options['agg_to_water'])
        dataset_test = GenDEBRIS('val', transform=transform_test,
                                 standardization=standardization, agg_to_water=options['agg_to_water'])
    ...
    # select gpu
    torch.cuda.set_device(options['cuda'])
    device = torch.device(f"cuda:{options['cuda']}")
    ...
    if options['agg_to_water']:
        ...
        # Drop Mixed Water, Wakes, Cloud Shadows, Waves
    ...
    if options['mode'] == 'train':
        ...
        for epoch in range(start, epochs+1):
            ...
            i_board = 0
            for (image, target) in tqdm(train_loader, desc=f"training {epoch}"):
            ...
            logging.info("Training loss was: " +
            ...
            if epoch % eval_every == 0 or epoch == 1:
                ...
                with torch.no_grad():
                    ...
                    logging.info("\n")
                    logging.info("Test loss was: " +
                                 str(sum(test_loss) / test_batches))
                    logging.info("STATISTICS AFTER EPOCH " +
                                 str(epoch) + ": \n")
                    ...
                    writer.add_scalar('Recall/test macroRec',
                                      acc["macroRec"], epoch)
                    writer.add_scalar('Recall/test microRec',
                                      acc["microRec"], epoch)
                    writer.add_scalar('Recall/test weightRec',
                                      acc["weightRec"], epoch)

                    writer.add_scalar('F1/test macroF1', acc["macroF1"], epoch)
                    writer.add_scalar('F1/test microF1', acc["microF1"], epoch)
                    writer.add_scalar('F1/test weightF1',
                                      acc["weightF1"], epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume_from_epoch', default=0,
                        type=int, help='load model from previous epoch')

    parser.add_argument('--input_channels', default=11,
                        type=int, help='Number of input bands')
    parser.add_argument('--output_channels', default=11,
                        type=int, help='Number of output classes')
    parser.add_argument('--hidden_channels', default=16,
                        type=int, help='Number of hidden features')
    ...
    parser.add_argument(
        '--lr_steps', default='[40]', type=str, help='Specify the steps that the lr will be reduced')

    # Evaluation/Checkpointing
    parser.add_argument('--checkpoint_path', default=os.path.join(up(os.path.abspath(__file__)),
                        'trained_models'), help='folder to save checkpoints into (empty = this folder)')
    ...
    parser.add_argument('--pin_memory', default=True,
                        type=bool, help='Use pinned memory or not')
    ...
    parser.add_argument('--tensorboard', default='tsboard_segm',
                        type=str, help='Name for tensorboard run')
    parser.add_argument('--cuda', default=0, type=int,
                        help="Index of CUDA Machine")
